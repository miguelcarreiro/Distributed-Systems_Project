	...
	public boolean handleMessage(SOAPMessageContext smc) {
		...
		try {
			if (outboundElement.booleanValue()) {
				...
				Date recvDate = dateFormatter.parse(valueString);

				// print received header
				System.out.println("Header value is " + valueString);
				
				System.out.println("Date diff: " + (new Date().getTime() - recvDate.getTime()));
				
				if(new Date().getTime() - recvDate.getTime() > 3000){
			...
			System.out.println("Message is not fresh!");
			throw new RuntimeException();
		} catch (Exception e) {
